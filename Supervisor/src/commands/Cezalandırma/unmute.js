...
if(!ayar) return;  
    ...
    {
    message.react(message.guild.emojiGöster(emojis.no))
    message.reply( { content:"Bu üye muteli değil!"}).then((e) => setTimeout(() => { e.delete(); }, 15000));   
    return }
    if (!message.member.permissions.has(8n) && member.roles.highest.position >= message.member.roles.highest.position) 
    {
    message.react(message.guild.emojiGöster(emojis.no))
    message.reply({ content:"Kendinle aynı yetkide ya da daha yetkili olan birinin susturmasını kaldıramazsın!"}).then((e) => setTimeout(() => { e.delete(); }, 15000)); 
    return }
    if (!member.manageable) 
    {
    message.react(message.guild.emojiGöster(emojis.no))
    message.reply({ content:"Bu üyenin susturmasını kaldıramıyorum!"}).then((e) => setTimeout(() => { e.delete(); }, 15000)); 
    return }
    
    message.react(message.guild.emojiGöster(emojis.yes))
    member.roles.remove(ayar.muteRoles).catch(e => {});
    const data = await penals.findOne({ userID: member.user.id, guildID: settings.guildID, type: "CHAT-MUTE", active: true });
    if (data) {
      data.active = false;
      await data.save();
    }
   message.reply({ content:`${message.guild.emojiGöster(emojis.unmute)} ${member.toString()} Üyesinin **Text Kanallarındaki** Susturması <t:${Math.floor(Date.now() / 1000)}:R> ${message.author} Tarafından Kaldırıldı!`})   
    if (settings.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.member.user.globalName ? message.member.user.globalName : message.member.user.tag}** tarafından susturmanız kaldırıldı!`}).catch(() => {});
...
.setThumbnail(member.user.avatarURL({dynamic: true}))
    .setTitle("Bir Kullanıcının Yazı Kanallarındaki Cezası Kaldırıldı")
      .setDescription(`
