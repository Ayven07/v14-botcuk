...
if (message.author.bot || message.member && message.member.permissions.has(Discord.PermissionsBitField.Flags.Administrator) || message.channel.type == Discord.ChannelType.Dm) return;
...
if (data.swearShield == true) {
if(await mainShield.checkWhitelist(message.author.id, "swear") || await mainShield.checkWhitelist(message.author.id, "full")) return;
if(links) return;
if(message.deletable) message.delete().catch(e => {})
message.channel.send({content: `${message.author} *Bu Sunucuda CapsLock Engel Aktif.*`}).sil(15)
}
}
...
sonMesajlar(message, 6)
usersMap.delete(message.author.id);
message.member.roles.add(ayar.muteRoles[0]).catch(e => {})
await message.channel.send({content: `${message.author} Sohbet kanallarını kirletme sebebiyle **1 dakika** süresince susturuldunuz, mesajlar temizlendi. Lütfen yavaşlayın.`}).sil(15)
setTimeout(() => {
if (message.member && !message.member.roles.cache.has(ayar.muteRoles[0])) return;
message.member.roles.remove(ayar.muteRoles[0]).catch(e => {
console.error("Rol kaldırılırken hata oluştu: " + e);
});
message.channel.send({ content: `${message.author} Sohbet kanallarını kirletme sebebiyle **1 dakika** süresince susturmanız bitti. Lütfen tekrarlamayınız.` }).sil(15);
}, 1000 * 60 * 1);
return 
} else {
...
if (data.swearShield == true && kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase())) && !["t-sustum"].some(ver => message.channel.name.includes(ver))) {
if(await mainShield.checkWhitelist(message.author.id, "swear") || await mainShield.checkWhitelist(message.author.id, "full") || args.some(c => c.toLowerCase() == "düdük")) return;
message.delete().catch(e => {})
message.channel.send({ content: `${message.author} *Bu Sunucuda Küfür Engel Aktif.*`}).then((e) => setTimeout(() => { e.delete(); }, 10000)).catch(err => {});
}
if (data.advertShield == true && kelime.some(reklam => message.content.toLowerCase().includes(reklam)) && ![chatShield.guilds.cache.get(settings.guildID).premiumSubscriptionCount > 14 && message.guild.vanityURLCode].some(reklam => message.content.toLocaleLowerCase().includes(reklam))) {
if(await mainShield.checkWhitelist(message.author.id, "advert") || await mainShield.checkWhitelist(message.author.id, "full")) return;
if (message.deletable) message.delete().catch(err => {});
message.channel.send({content: `${message.author} *Bu Sunucuda Reklam Engel Aktif.*` }).sil(15)
} else {
if (!links) return;
if(data.advertShield == true) {  
if(await mainShield.checkWhitelist(message.author.id, "advert") || await mainShield.checkWhitelist(message.author.id, "full")) return;  
