...
let logs = await channel.guild.fetchAuditLogs({ type: Discord.AuditLogEvent.ChannelCreate });
...
**❯** ${entry.executor} Bir **Kanal Açıldı!** Üye Güvenli Listede Olduğu İçin **İşlem Uygulanmadı!**
           
\`••❯\` Kanalı Açan Kişi: ${entry.executor} \`(${entry.executor.id})\`
\`••❯\` Açılan Kanal: ${channel.name} \`(${channel.id})\` [\`${tür[channel.type]}\`]
...
channel.delete({ reason: `İzinsiz Kanal Açma İşlemi!` }).catch(e => {})
