...
if(await mainShield.checkWhitelist(newMessage.author.id, "swear") || await mainShield.checkWhitelist(newMessage.author.id, "full")) return;
if(links) return; 
if(newMessage.deletable) newMessage.delete().catch(e => {})
newMessage.channel.send({content: `${newMessage.author} *Bu Sunucuda CapsLock Engel Aktif.*`}).sil(15)
}
}
function percentage(partialValue, totalValue) {
return (100 * partialValue) / totalValue;
} 
if (data.swearShield == true && kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase())) && !["sus"].some(ver => newMessage.channel.name.includes(ver))) {
if(await mainShield.checkWhitelist(newMessage.author.id, "swear") || await mainShield.checkWhitelist(newMessage.author.id, "full")) return;
newMessage.delete().catch(e => {}).then(message => {
newMessage.channel.send({ content: `${newMessage.author} *Bu Sunucuda Küfür Engel Aktif.*`}).then((e) => setTimeout(() => { e.delete(); }, 10000)).catch(err => {});
});
}
if (data.advertShield == true && kelime.some(reklam => newMessage.content.toLowerCase().includes(reklam)) && ![newMessage.guild.vanityURLCode].some(reklam => newMessage.content.toLocaleLowerCase().includes(reklam))) {
if(await mainShield.checkWhitelist(newMessage.author.id, "advert") || await mainShield.checkWhitelist(newMessage.author.id, "full")) return;
if (newMessage.deletable) newMessage.delete().catch(err => {});
newMessage.channel.send({content: `${newMessage.member} *Bu Sunucuda Reklam Engel Aktif.*` }).sil(15)
} else {
if (!links) return;
